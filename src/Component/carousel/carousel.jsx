...
// console.log(imageList, onSave);
  ...
  return (
    <div className="carousel-conatiner">
      <div ref={carouselRef} className="image-container">
        {imageList.length ? (
          imageList.map((image, index) => {
            return (
              <img className="carousel-image" src={image.url} key={index} />
            );
          })
        ) : (
          <img
            className="carousel-image"
            src="https://upload.wikimedia.org/wikipedia/commons/6/65/No-Image-Placeholder.svg" alt="loading..."
          />
        )}
      </div>
      ...
      <div className="carousel.circle-conatiner">{}</div>
    </div>
  );
}
